    ...
    div [ class "page become-organizer-page" ]
        ...
        , case model.wallet of
            ...
            Just _ ->
                if model.isOrganizer then
                    div []
                        [ p [ class "help-text" ] [ text "✅ You are already registered as an organizer!" ]
                    ...
                    div []
                        [ p [] [ text "Register as an event organizer to unlock these features:" ]
                        , ul []
                            [ li [] [ text "Create events with custom pricing and ticket supply" ]
                            , li [] [ text "Manage your events (cancel, issue refunds)" ]
                            , li [] [ text "Check in attendees at your events" ]
                            , li [] [ text "Track ticket sales and revenue" ]
                            ]
                        , p [ class "help-text" ]
                            [ text "Registration is free and only requires a small transaction fee (~0.001 SOL for rent, which is refundable)." ]
                        , button [ onClick RegisterOrganizer, class "primary-button" ] [ text "Register as Organizer" ]
                        ]
        ]
...
viewCheckInPage model =
    div [ class "page checkin-page" ]
        ...
        , p [ class "help-text" ]
            [ text "⚠️ This page is for EVENT STAFF ONLY." ]
        , p [ class "help-text" ]
            [ text "Only the event authority (organizer) can check in tickets." ]
        , div [ class "instructions-box" ]
            [ h3 [] [ text "🔐 Wallet Verification Process:" ]
            , ol []
                [ li [] [ text "Ask the attendee for their wallet address" ]
                , li [] [ text "Verify they control that wallet by asking them to:" ]
                , ul []
                    [ li [] [ text "Show their wallet app with the address visible, OR" ]
                    , li [] [ text "Send a tiny amount (e.g., 0.001 SOL) from their wallet to prove ownership" ]
                    ]
                , li [] [ text "Look up tickets owned by that wallet address on-chain" ]
                , li [] [ text "Confirm the ticket is valid (not used, not refunded, correct event)" ]
                , li [] [ text "Enter the ticket address below and click 'Check In Ticket'" ]
                ]
            , p [ class "warning-text" ]
                [ text "⚠️ Important: Anyone can see ticket addresses on the blockchain, but only the wallet owner can prove they control the wallet that owns the ticket." ]
            ]
        , case model.wallet of
            Nothing ->
                p [ class "help-text" ] [ text "Please connect your wallet first." ]
